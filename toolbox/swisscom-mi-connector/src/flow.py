...
def swisscom_mi_connector():
    flow = Flow(
        ...
        {% endif %}
        # Save the results
        add_metadata(name='swisscom_mi_connector', title='''{{title}}'''),
        ...
        dump_to_path('swisscom_mi_connector'),
        {% endif %}
        {% if output == 'dp_csv_zip' %}
        dump_to_zip('swisscom_mi_connector.zip'),
        {% endif %}
        {% if output == 'dp_json' %}
        dump_to_path('swisscom_mi_connector', force_format=True, format='json'),
        {% endif %}
        {% if output == 'dp_json_zip' %}
        dump_to_zip('swisscom_mi_connector.zip', force_format=True, format='json'),
        {% endif %}
        {% if output == 'sql' %}
